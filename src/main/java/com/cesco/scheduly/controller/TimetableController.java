...
import java.util.Collections; // Collections.emptyList() 사용 위함
...
public class TimetableController {
    ...
    public ResponseEntity<ApiResponse> updateUserTimePreferences(
        ...
        try {
            ...
            // Log error e
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiResponse("시간 선호도 저장 중 서버 오류 발생"));
        }
    }
    ...
    public ResponseEntity<ApiResponse> updateUserCreditAndCombinationPreferences(
        ...
        try {
            ...
            // Log error e
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiResponse("학점/조합 설정 저장 중 서버 오류 발생"));
        }
    }
    ...
    public ResponseEntity<?> getRecommendedTimetables(@PathVariable String userId) {
        try {
            ...
            if (recommendations == null || recommendations.isEmpty()) {
                // 빈 리스트와 메시지를 함께 보내는 것이 더 명확할 수 있습니다.
